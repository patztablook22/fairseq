...
class Trainer(object):
    ...
    def _build_optimizer(self):
        ...
        if self.cfg.checkpoint.parameter_freeze_substr is not None:
            substrings = self.cfg.checkpoint.parameter_freeze_substr.strip("~").split(",")
            if self.cfg.checkpoint.parameter_freeze_substr[0] == "~":
                frozen_params = list(
                    filter(
            ...
            logger.info(
                "Following parameter weights will be frozen during training: "
                "{}".format([p[0] for p in frozen_params])
            )
