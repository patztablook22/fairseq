...
def _make_binary_image_dataset(
    input_prefix: str, output_prefix: str, num_workers: int, args: Namespace
):
    logger.info(
        "[images] {}: parsing...".format(
            input_prefix
        )
    )
    ...
    if num_workers != 1:
        logger.warning(
            "{} currently does not support multithread processing. "
            "Using single thread...".format(binarizer.__class__.__name__)
        )
    final_summary = FileBinarizer.multiprocess_dataset(
        input_file,
        args.dataset_impl,
...
def _make_all_alignments(args):
    ...
    if args.validpref:
        for k, validpref in enumerate(args.validpref.split(",")):
            if os.path.exists(validpref + "." + args.align_suffix):
                outprefix = "valid{}.align".format(k) if k > 0 else "valid.align"
                _make_binary_alignment_dataset(
                    validpref + "." + args.align_suffix,
                    outprefix,
                    num_workers=args.workers,
                    args=args,
                )
    if args.testpref:
        for k, testpref in enumerate(args.testpref.split(",")):
            if os.path.exists(testpref + "." + args.align_suffix):
                outprefix = "test{}.align".format(k) if k > 0 else "test.align"
                _make_binary_alignment_dataset(
                    testpref + "." + args.align_suffix,
                    outprefix,
                    num_workers=args.workers,
                    args=args,
                )
...
def _make_all_images(args):
    if args.trainpref and os.path.exists(args.trainpref + "." + args.image_suffix):
        _make_binary_image_dataset(
            args.trainpref + "." + args.image_suffix, 
            "train.image",
            num_workers=args.workers,
            args=args,
        )
    if args.validpref:
        for k, validpref in enumerate(args.validpref.split(",")):
            if os.path.exists(validpref + "." + args.image_suffix):
                outprefix = "valid{}.image".format(k) if k > 0 else "valid.image"
                _make_binary_image_dataset(
                    validpref + "." + args.image_suffix,
                    outprefix,
                    num_workers=args.workers,
                    args=args,
                )
    if args.testpref:
        for k, testpref in enumerate(args.testpref.split(",")):
            if os.path.exists(testpref + "." + args.image_suffix):
                outprefix = "test{}.image".format(k) if k > 0 else "test.image"
                _make_binary_image_dataset(
                    testpref + "." + args.image_suffix,
                    outprefix,
                    num_workers=args.workers,
                    args=args,
                )
