...
OUTDIR=$WD/wmt20_${SRC}${TGT}.labeled.$BUCKET_CRITERION
TMPDIR=$OUTDIR/tmp.d
...
for max_len in $BUCKETS; do
    ...
    paste $OUTDIR/bpe.train.$SRC $OUTDIR/bpe.train.$TGT | \
        ...
        ~/scripts/filter_sentpairs_by_length.py \
            --min-length $min_len \
            --max-length $max_len \
            --add-max-length-label \
            --field $BUCKET_CRITERION > $OUTDIR/bpe.$max_len.train
    cut -f1,2 $OUTDIR/bpe.$max_len.train | \
        tr "\t" " " > $OUTDIR/bpe.$max_len.train.$SRC
    cut -f3 $OUTDIR/bpe.$max_len.train \
    ...
    for t in "valid" "test"; do
        ...
        cut -f1,2 $OUTDIR/bpe.newstest.$max_len.$t | \
            tr "\t" " " | \
            sed 's/@@ //g' | \
            perl $DETOKENIZER -l $SRC \
            > $OUTDIR/newstest.$max_len.$t.$SRC
        cut -f3 $OUTDIR/bpe.newstest.$max_len.$t | \
            sed 's/@@ //g' | \
...
f_out=$TMPDIR/czeng.train
rm $f_out
for len in $BUCKETS; do
    paste $OUTDIR/czeng.$len.train.$SRC $OUTDIR/czeng.$len.train.$TGT >> $f_out
done
cat $f_out | shuf > $f_out.shuf
cut -f1 $f_out.shuf > $OUTDIR/czeng.train.$SRC
cut -f2 $f_out.shuf > $OUTDIR/czeng.train.$TGT

for t in "valid" "test"; do
    f_out=$TMPDIR/newstest.$t
    rm $f_out
    for len in $BUCKETS; do
        paste $OUTDIR/newstest.$len.$t.$SRC $OUTDIR/newstest.$len.$t.$TGT >> $f_out
    done
    cat $f_out | shuf > $f_out.shuf
    cut -f1 $f_out.shuf > $OUTDIR/newstest.$t.$SRC
    cut -f2 $f_out.shuf > $OUTDIR/newstest.$t.$TGT
done
