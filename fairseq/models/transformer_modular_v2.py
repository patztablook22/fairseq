...
@register_model("transformer_modular_v2")
class TransformerModularModelV2(TransformerModel):
    """
    TODO
    ...
    def add_args(parser):
        ...
        # fmt: off
        super(TransformerModularModelV2, TransformerModularModelV2).add_args(parser)
        ...
        parser.add_argument('--e-step-size', type=int, metavar='N',
                            help='size of the expectation step')
        parser.add_argument('--m-steps', type=int, metavar='N',
                            help='number of maximization steps between the e-steps')
        parser.add_argument('--m-warmup', type=int, metavar='N',
                            help='number of warmup steps before periodic e-step application start')
        # fmt: on
    ...
    def initialize_best_selection(self, dataset_size):
        for i, _ in enumerate(self.encoder.layers):
            if isinstance(
                self.encoder.layers[i], TransformerModularEncoderLayerV2
            ):
                self.encoder.layers[i].initialize_best_selection(dataset_size)

    def update_best_selection(self, selections, indices):
        assert len(selections) == len(self.encoder.layers)
        for i, sel in enumerate(selections):
            if sel is not None:
                self.encoder.layers[i].update_best_selection(sel, indices)
    ...
    def get_best_selection(self, indices):
        ...
        for i, layer in enumerate(self.encoder.layers):
            if isinstance(
                ...
                selections.append(self.encoder.layers[i].get_best_selection(indices))
            else:
...
class TransformerModularEncoder(TransformerEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        for i in range(args.encoder_layers):
            if i in modular_layer_indices:
                self.layers.append(TransformerModularEncoderLayerV2(args))
            else:
    ...
    def forward_layer_wrapper(self,
                              ...
                              mode,
                              indices):
        if not isinstance(layer, TransformerModularEncoderLayerV2):
            return layer(x, encoder_padding_mask), None, None
        else:
            return layer(x, encoder_padding_mask, mode, indices)
        ...
        for layer in self.layers:
            ...
            if not self.training or (dropout_probability > self.encoder_layerdrop):
                x, selection, ctrl = self.forward_layer_wrapper(
                    layer, x, encoder_padding_mask, mode, indices)
...
@register_model_architecture("transformer_modular_v2", "transformer_modular_v2")
def transformer_modular(args):
    ...
    args.e_step_size = getattr(args, 'e_step_size', 10)
    args.m_steps = getattr(args, 'm_steps', 10)
    args.m_warmup = getattr(args, 'm_warmup', 0)
