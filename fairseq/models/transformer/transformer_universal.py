...
@register_model("transformer_universal", dataclass=TransformerUniversalConfig)
class TransformerUniversalModel(TransformerModelBase):
    """
    TODO
    ...
    Args:
        encoder (TransformerUniversalEncoder): the encoder
        decoder (TransformerUniversalDecoder): the decoder
    ...
    def add_args(cls, parser):
        ...
        gen_parser_from_dataclass(
            parser, TransformerUniversalConfig(), delete_default=False, with_prefix=""
        )
    ...
    def build_encoder(cls, cfg, src_dict, embed_tokens):
        return TransformerUniversalEncoder(cfg, src_dict, embed_tokens)

    @classmethod
    def build_decoder(cls, cfg, tgt_dict, embed_tokens):
        return TransformerUniversalDecoder(
            cfg,
            tgt_dict,
            embed_tokens,
...
class TransformerUniversalEncoder(FairseqEncoder):
    """
    ...
    def build_encoder_layer(self, cfg):
        layer = transformer_universal_layer.TransformerUniversalEncoderLayer(
    ...
    def forward_scriptable(
        ...
        vert_finished_mask = (
            encoder_padding_mask.unsqueeze(-1).type_as(x)
        ).type(torch.bool)
        x = x * (
...
class TransformerUniversalDecoder(FairseqIncrementalDecoder):
    """
    ...
    def build_decoder_layer(self, cfg, no_encoder_attn=False):
        layer = transformer_universal_layer.TransformerUniversalDecoderLayer(
            cfg, no_encoder_attn
        )
        checkpoint = cfg.checkpoint_activations
        if checkpoint:
