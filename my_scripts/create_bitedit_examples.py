...
def create_id_example(x, mapping, tok=None, n=None):
    return (x, "-", "-")


def create_push_example(x, mapping, tok, n=1):
    # push
    return (x + [tok] * n, tok, str(n))


def create_pop_example(x, mapping, tok=None, n=1):
    # pop
    return (x[:-n], "-", str(n))


def create_unshift_example(x, mapping, tok, n=1):
    # unshift
    return ([tok] * n + x, tok, str(n))


def create_shift_example(x, mapping, tok=None, n=1):
    # shift
    return (x[n:], "-", str(n))


def create_remove_tokens_example(x, mapping, tok, n=None):
    # remove
    return ([e for e in x if e != tok], tok, "-")


def create_reverse_example(x, mapping, tok=None, n=None):
    # reverse
    y = x.copy()
    y.reverse()
    return (y, "-", "-")
...
def create_duplicate_example(x, mapping, tok=None, n=None):
    # duplicate
    return (x + x, "-", "-")
...
def main(args):
    ...
    for i, x in enumerate(x_all):
        x_bin = binarize(x, mapping)

        fn = TASK_MAP[args.task]
        if i % 2 == 0:
            y_bin, tok, n = fn(x_bin, mapping, args.zero_char, args.n)
        else:
            y_bin, tok, n = fn(x_bin, mapping, args.one_char, args.n)
        print("{} {} {}\t{}\t{}".format(
