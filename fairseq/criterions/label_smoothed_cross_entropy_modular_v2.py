...
@register_criterion('label_smoothed_cross_entropy_modular_v2')
class LabelSmoothedCrossEntropyModularV2Criterion(FairseqCriterion):

    def __init__(self, args, task):
        super().__init__(args, task)
        self.eps = args.label_smoothing
        
        self.e_step_size = args.e_step_size
        self.m_steps = args.m_steps
        self.m_warmup = args.m_warmup

        self.step_id = 1  # indicate whether to apply e-step
        assert self.e_step_size >= 0
        assert self.m_steps > 0
        assert self.m_warmup >= 0

        self.old_best_sel = None
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if self.step_id % (self.m_steps + 1) == 0 and self.m_warmup <= 0:
            with torch.no_grad():
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        loss = loss.view(bsz, -1).sum(1) + sel_loss
